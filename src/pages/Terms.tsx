...
const articles: { title: string; body: string[] }[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "월권(자유석·지정석)은 4주마다 자동으로 결제되는 정기결제로 이용할 수 있으며, 회원은 ‘내정보 > 예약현황’에서 언제든 직접 해지할 수 있습니다. 다만 카드사 사정으로 BC카드는 정기결제 등록이 제한되며, 이 경우 이용 기간마다 결제하여 이용합니다.",
  ...
  },
  {
    ...
    body: [
      ...
      "정기결제(월권): 회원은 다음 결제일 전 언제든 해지할 수 있고 해지 후 추가 결제는 발생하지 않으며, 이미 결제한 이용 기간은 종료일까지 이용할 수 있습니다.",
      "주간권·월권 환불: 이용 기간 시작 전 취소 시 전액 환불합니다. 이용을 시작한 뒤에는 주 단위로 정산하여, 이용을 시작한 주는 이용한 것으로 보고 남은 주에 해당하는 금액을 환불합니다. (예: 4주 이용권을 2주차에 해지하면 남은 2주분 환불)",
      "다만 전체 이용 기간이 1주 이하인 이용권(주간권 등)은 일 단위로 정산하여 남은 일수에 해당하는 금액을 환불합니다.",
      "주 단위로 정산하는 이유: 주간권·월권은 해당 기간 동안 좌석을 배정해 두는 상품으로(자유석도 단독석 위주로 배정) 중도 해지 시 남은 기간의 좌석을 다시 판매하기 어렵기 때문입니다.",
      "이용 기간 중 환불은 화면에서 바로 처리되지 않으며, 회사에 문의하여 처리합니다.",
