  ...
  return (
    <main className="pb-12">
      <Section eyebrow="Admin" title="회원 신청 관리">
        <div className="mb-5 grid gap-3 rounded-card border-2 border-workroom-line bg-workroom-surface p-4 shadow-soft sm:grid-cols-[1fr_auto_auto] sm:items-end">
          ...
          <button className="rounded-full border-2 border-workroom-line bg-workroom-yellow px-5 py-3 font-black" onClick={loadMembers} type="button">
            새로고침
          </button>
          <Link className="rounded-full border-2 border-workroom-line bg-white px-5 py-3 text-center font-black" to="/admin/reservations">
            예약관리
          </Link>
        </div>

        <p className="mb-4 rounded-card border-2 border-workroom-line bg-workroom-yellow p-4 text-sm font-black">
          승인 대기 회원 {pendingCount}명
        </p>
        {isLoading ? <p className="rounded-card border-2 border-workroom-line bg-workroom-yellow p-4 font-black">회원 신청을 불러오는 중입니다.</p> : null}
        ...
        {!isLoading && !visibleMembers.length ? (
          <p className="rounded-card border-2 border-workroom-line bg-workroom-surface p-6 text-center font-black shadow-sketch">조건에 맞는 회원이 없습니다.</p>
        ) : null}

        <div className="grid gap-4 lg:grid-cols-2">
          {visibleMembers.map((member) => (
            <article className="rounded-card border-2 border-workroom-line bg-workroom-surface p-5 shadow-sketch" key={member.id}>
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-full border-2 border-workroom-line bg-workroom-yellow px-3 py-1 text-xs font-black">
                  {member.role === "admin" ? "관리자" : statusLabels[member.membership_status]}
                </span>
              </div>
              {member.address ? <p className="mt-4 rounded-card border-2 border-workroom-line bg-white p-3 text-sm font-black">{member.address}</p> : null}
              <div className="mt-5 grid grid-cols-3 gap-2">
                {memberStatuses.map((status) => (
                  <button
                    className={`rounded-full border-2 border-workroom-line px-3 py-2 text-sm font-black ${
