...
export const SITE = {
  ...
  booking: {
    ...
    recurringNotice: recurringPaymentLive
      ? `월권(자유석·지정석)은 4주마다 자동으로 결제되는 정기결제로 이용할 수 있으며, ‘내정보 > 예약현황’에서 언제든 해지할 수 있습니다. 회차별 금액은 결제하는 날 기준으로, 할인이 걸려 있으면 할인가로, 없으면 정가로 청구되며 등록 시 금액을 넘지 않습니다. 다만 ${recurringUnsupportedCards}는 카드사 사정으로 정기결제 등록이 불가하므로, 이용 기간마다 카드로 결제해 주세요.`
      : "월권(자유석·지정석)은 이용 기간마다 카드로 결제하시면 됩니다. 4주마다 자동으로 결제되는 정기결제는 준비 중입니다.",
    // 정기결제 버튼 옆에 붙는 짧은 안내.
    recurringHint: recurringPaymentLive
      ? `카드가 등록되면 첫 회차가 바로 결제되고 이후 4주마다 자동 결제돼요. 언제든 해지할 수 있어요. 할인 중에 등록하시면 할인 기간에는 할인가로, 할인이 끝나면 정가로 결제됩니다. ${recurringUnsupportedCards}는 정기결제 등록이 불가해서, 그 경우 위의 ‘카드로 결제하기’를 이용해 주세요.`
